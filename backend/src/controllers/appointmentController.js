          ...
          (scheduleErr, withinSchedule) => {
            ...
            if (!withinSchedule) {
              return res.status(400).json({
                success: false,
                message:
                  'Nhân viên không làm việc vào khung giờ này theo lịch hằng tuần, vui lòng chọn thời gian khác hoặc nhân viên khác'
              });
            }
...
exports.getMyAppointments = (req, res) => {
  ...
  if (role === 'staff') {
    // For staff, get appointments assigned to them
    appointmentModel.getAppointmentsByStaffId(req.user.id, (err, appointments) => {
      if (err) {
        return res.status(500).json({ success: false, message: 'Lôi server', error: err });
      }
